...
export default async function ApiReferencePage(props: {
  ...
  const MDX = doc.body;
  const neighbours = await findNeighbour(apiReference.pageTree, page.url);

  // @ts-expect-error - revisit fumadocs types.
  const links = doc.links;
  console.log(apiReference.pageTree);
  return (
    <DocsPage
      // @ts-expect-error - revisit fumadocs types.
      toc={page.data.toc}
      ...
      full={page.data.full}
      breadcrumb={{
        component: <DocsBreadcrumb tree={apiReference.pageTree} />,
      }}
      tableOfContent={{
        style: "clerk",
