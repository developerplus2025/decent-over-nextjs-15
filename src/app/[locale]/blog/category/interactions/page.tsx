  ...
  return (
    <main className="mb-16 flex w-full flex-col items-center justify-center px-40">
      ...
      <div className="mt-12 space-y-8">
        <div className="grid justify-items-center divide-x divide-y border-r sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 102 }).map((_, index) => {
            const post = posts[index % posts.length]; // Lặp lại danh sách khi hết dữ liệu
            return (
              <Card
                key={index}
                className={`flex flex-col justify-between overflow-hidden rounded-none ${index === 0 ? "border-0 border-l border-t" : "border-0"}`}
              >
